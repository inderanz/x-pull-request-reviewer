    ...
    for root, _, files in os.walk(directory):
        for file in files:
            ...
            if lang == 'terraform':
                ...
                fmt = yaml_adapter.run_prettier_check(path)
                lint = yaml_adapter.run_yamllint(path)
                summary[path] = {'format': fmt, 'lint': lint}
            elif lang == 'go':
                fmt = go_adapter.run_gofmt_check(path)
                lint = go_adapter.run_golint(path)
                summary[path] = {'format': fmt, 'lint': lint}
            elif lang == 'java':
                fmt = java_adapter.run_google_java_format_check(path)
                lint = java_adapter.run_checkstyle(path)
                summary[path] = {'format': fmt, 'lint': lint}
            elif lang == 'shell':
                fmt = shell_adapter.run_shfmt_check(path)
